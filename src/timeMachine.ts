...
class TimeMachine {
  ...
  private _v(state, fn = (state, allStates) => state) {
    return lastState => makePair(state, () => this._v(fn(state, lastState), fn)(lastState))
  }

  public onBoard = (name: passager['name'], state: passager['state'], fn?: (state, allStates?) => any): TimeMachine => {
    if (this._passagers.get(name)) throw new Error('Passager is already onboard.')
